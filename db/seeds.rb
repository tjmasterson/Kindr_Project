...
native_roots_location = Location.create(address: "1555 Champa St,Denver,CO", dispensary_id: native_roots.id)
...
grape_ape = Strain.create(name: "Grape Ape", photo_url: "cherryPie.jpg")
...
death_star = Strain.create(name: "Death Star", photo_url: "greenCrack.jpg")
...
effects_array = [lazy, focused, sleepy, giggly, happy, creative, sleepy, talkative, energetic, euphoric, uplifted]

medical_array = [insomnia, fatigue, pain, glaucoma, muscle_spasms, anxiety, depression, stress, nausea]

negative_array = [shakes, dry_mouth, paranoia, dry_eyes, dizzy, headache]

DispensaryStrain.all.each do |x|
  30.times do
    # x.user_choices.create(choice_id: overall_rating.id, rating: (1..10).to_a.sample)
    x.user_choices.create(choice_id: effects_array.sample.id, rating: (1..10).to_a.sample)
    x.user_choices.create(choice_id: medical_array.sample.id, rating: (1..10).to_a.sample)
    x.user_choices.create(choice_id: negative_array.sample.id, rating: (1..10).to_a.sample)
  end
end
